...
detached form is a task — `submit --agent bash --task '…'`. There is no
`--detach` on `exec`, and that is deliberate: one existed briefly and was
removed, because on Linux a normally-completing exec already leaves its
background children running, so the flag bought nothing there and only put them
out of `exec kill`'s reach. On Windows a command cannot detach itself either —
the group is a job object and `CREATE_BREAKAWAY_FROM_JOB` is denied — so if the
need ever comes back it has to keep the group killable rather than skip it.

`--sshd-parent` needs `--shell`: it runs the line under a process **named**
`sshd`, for a client that decides whether it is talking to an SSH server by
walking its own ancestry and comparing process names. Wired on Windows only —
elsewhere the runner refuses rather than running the command without the
property and reporting success. A long one is
...
and alongside at most one attach mode it takes `sshd-parent` — the `exec` option
above, applied to every command this connection runs. It lives in the user name
because that is the only thing that reaches the gateway from a client building
its own ssh invocation: a `~/.ssh/config` `User` line is written once and covers
every exec the connection makes, which is the right granularity for a property
of what the connection is *for*. Order does not matter and the forms compose, so
`.control,sshd-parent` is a valid name.
