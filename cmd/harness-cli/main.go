...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  session snapshot [--rows N] [--cols N] [--settle-ms MS] [--style] [--color] [--json] [--raw] TASK_ID")
	fmt.Fprintln(os.Stderr, "                                      print the session's current PTY screen as text (view attach; non-intrusive, works without a TTY)")
	fmt.Fprintln(os.Stderr, "                                      --style/--color append attribute/color spans; --json emits {rows,cols,lines[],spans[]} instead of text")
	fmt.Fprintln(os.Stderr, "                                      --raw writes the verbatim PTY bytes instead of the VT render (not combinable with --style/--color/--json)")
