...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  session snapshot [--rows N] [--cols N] [--settle-ms MS] [--style] [--color] [--raw] TASK_ID")
	fmt.Fprintln(os.Stderr, "                                      print the session's current PTY screen as text (view attach; non-intrusive, works without a TTY)")
	fmt.Fprintln(os.Stderr, "                                      --style/--color append attribute/color spans; --raw writes the verbatim PTY bytes instead of the VT render")
...
func agentUsage() {
	...
	fmt.Fprintln(os.Stderr, "  send --in-reply-to SEQ D|-          reply to SEQ; --topic optional (server routes to that message's sender)")
	...
	fmt.Fprintln(os.Stderr, "                                       WITHOUT it the server returns an EMPTY list, not an error, so an")
	fmt.Fprintln(os.Stderr, "                                       empty result here does not mean the board is empty)")
