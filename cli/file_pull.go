...
func (c *Client) FilePull(ctx context.Context, taskIDHex, remoteRel, localPath string, rng FileTransferRange, force, dataPlane bool) error {
	return c.filePullDo(ctx, taskIDHex, remoteRel, rng, dataPlane, func(stream trsf.BidirectionalStream, expectedSize, _ uint64) error {
...
func (c *Client) FilePullBytes(ctx context.Context, taskIDHex, remoteRel string, dataPlane bool, onProgress ProgressFunc) ([]byte, error) {
	var buf bytes.Buffer
	if err := c.filePullDo(ctx, taskIDHex, remoteRel, FileTransferRange{}, dataPlane, func(stream trsf.BidirectionalStream, expectedSize, _ uint64) error {
...
func (c *Client) filePullDo(ctx context.Context, taskIDHex, remoteRel string, rng FileTransferRange, dataPlane bool, body func(stream trsf.BidirectionalStream, n, total uint64) error) error {
	stream, err := c.OpenFileTransfer(ctx, taskIDHex, protocol.FileTransferDirection_Pull, remoteRel, 0, rng, false, false, dataPlane)
	if err != nil {
		return err
	}
...
func (c *Client) FilePullBytesRange(ctx context.Context, taskIDHex, remoteRel string, rng FileTransferRange, dataPlane bool, onProgress ProgressFunc) ([]byte, uint64, error) {
	var buf bytes.Buffer
	var total uint64
	if err := c.filePullDo(ctx, taskIDHex, remoteRel, rng, dataPlane, func(stream trsf.BidirectionalStream, n, tot uint64) error {
		total = tot
		buf.Grow(int(n))
...
func (c *Client) FilePullDir(ctx context.Context, taskIDHex, remoteRel, localDir string, force, dataPlane bool) error {
	...
	stream, err := c.OpenFileTransfer(ctx, taskIDHex, protocol.FileTransferDirection_DirPull, remoteRel, 0, FileTransferRange{}, false, false, dataPlane)
	if err != nil {
		return err
	}
	defer stream.CloseBoth()
...
func (c *Client) FilePullDirBytes(ctx context.Context, taskIDHex, remoteRel string, dataPlane bool, onProgress ProgressFunc) ([]byte, error) {
	stream, err := c.OpenFileTransfer(ctx, taskIDHex, protocol.FileTransferDirection_DirPull, remoteRel, 0, FileTransferRange{}, false, false, dataPlane)
	if err != nil {
		return nil, err
	}
