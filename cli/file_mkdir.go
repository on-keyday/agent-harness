...
// closes.
func (c *Client) FileMkdir(ctx context.Context, taskIDHex, remoteRel string, parents, dataPlane bool) error {
	stream, err := c.OpenFileTransfer(ctx, taskIDHex, protocol.FileTransferDirection_Mkdir, remoteRel, 0, FileTransferRange{}, false, parents, dataPlane)
	if err != nil {
		return err
	}
