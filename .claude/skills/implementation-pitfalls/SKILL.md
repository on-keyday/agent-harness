...
- **harness-cli never sync wait/dispatch from agent turn** (`feedback_no_sync_wait_dispatch`): `send` only and end the turn. Replies arrive via inbox hook on a later turn. Applies to the initial `hello` handshake too. Synchronous wait stalls the entire conversation.
