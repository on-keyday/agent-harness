...
func TestTopic_AppendInRing(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		topic.append(uint64(i+1), []byte{byte(i)}, zeroRid, zeroTid, "", "", 0, sendConfig{})
	}
	got := topic.since(0)
	if len(got) != 4 {
...
func TestTopic_SinceFiltersByCursor(t *testing.T) {
	...
	for i := uint64(1); i <= 5; i++ {
		topic.append(i, []byte{byte(i)}, zeroRid, zeroTid, "", "", 0, sendConfig{})
	}
	got := topic.since(2)
	if len(got) != 3 {
...
func TestTopic_LastPublishedAtUpdates(t *testing.T) {
	...
	var zeroTid protocol.TaskID
	t0 := time.Now()
	topic.append(1, []byte("a"), zeroRid, zeroTid, "", "", 0, sendConfig{})
...
func TestTopic_AppendCarriesSender(t *testing.T) {
	...
	tp.append(1, []byte("hi"), rid, tid, "host-A", "", 0, sendConfig{})
...
func TestTopic_Append_CarriesInReplyTo(t *testing.T) {
	...
	var zeroTid protocol.TaskID
	tp := newTopic("t", 4)
	tp.append(1, []byte("parent"), zeroRid, zeroTid, "h", "", 0, sendConfig{})
	tp.append(2, []byte("child"), zeroRid, zeroTid, "h", "", 1, sendConfig{})
	got := tp.snapshot()
	if len(got) != 2 {
