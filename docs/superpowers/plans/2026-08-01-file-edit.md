...
- Produces: `writeFileEditTemp(name, text string) (string, error)`, `editorExec` (a `tea.ExecCommand` implementation), `fileEditExecDoneMsg{path string, err error}` and the `App` state `fileEditTmpPath`.

**Why `editorExec` and not `tea.ExecProcess`:** `Program.exec` calls
`ReleaseTerminal` before running the command (`bubbletea/exec.go:101`), and that
leaves the alt screen (`bubbletea/tea.go:872-880`). Anything the popup rendered
is therefore invisible while the editor runs, so the "TUI is suspended, this is
expected" message has to come from the child side. bubbletea calls
`SetStdout(p.output)` on the `ExecCommand` right before `Run()`
(`bubbletea/exec.go:110-112`), so a custom implementation can print to the
terminal at exactly that moment. A GUI editor leaves the message on screen for
its whole lifetime; a terminal editor overpaints it at once.
...
Add `"bytes"` and `"os/exec"` to the test file's imports alongside the ones
above. `exec.Command("true")` is a no-op child that exists on this project's
Linux hosts; the assertion is about what was written, not what ran.
...
Also in `tui/fileedit.go`, the announcing exec wrapper:
...
Add `"fmt"`, `"io"`, `"os/exec"` to that file's imports. The `\r\n` is
deliberate: the terminal is still in raw mode when this runs, so a bare `\n`
would step down a line without returning the cursor to column 0.
