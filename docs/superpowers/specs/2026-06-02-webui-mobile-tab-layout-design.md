...
- 広幅ではタブが無く全セクション可視。`setActiveTab()` は基本ノーオペ（body の data 属性更新のみ、CSS media query で非表示効果は無効）だが、**端末をアクティブ化（Open / Reattach / Resume）したときは `#interactive` を `scrollIntoView` でビューに入れる**。広幅では端末セクションが操作系の下にあるため、attach 後に手動スクロールせず端末が見えるようにする（狭幅はタブ切替＋`scrollTo(0,0)` で対応済みのため除外）。
