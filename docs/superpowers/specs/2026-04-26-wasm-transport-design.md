...
|   |                          xterm.js + xterm.css           |
|   +- /ws       -> transport.WebSocketEndpoint(Mode=Server)  |
|                   が登録した accept handler                 |
|                                                             |
...
- **構造体化**: 引数の意味を field 名で明示するため、`WebSocketConfig` 単一構造体に統合。`Mode` フィールドで Client / Server / Mutual を区別する。
- **API 関数は単一**: `WebSocketEndpoint(mux, cfg)` と `WebSocketEndpointEx(rawSess, mux, cfg)` の 2 つだけ。mode 別に関数分離せず、関数内部で `cfg.Mode` を見て分岐。AI subagent 主体の作業を前提にすると、関数表面が小さい方が briefing と context window に優しい。dualstack 等の複合 caller が switch せずに 1 行で済む利点もある。誤用 (Client モードで mux を渡す等) は run-time check + subagent への明示指示で防ぐ。
- **mux/path を引数で受ける**（Server / Mutual モード時）: caller は `*http.ServeMux` を渡し、callee が `mux.Handle(cfg.Path, handler)` を実行する。caller の登録忘れを構造的に防ぐ。Client モード時は `mux` に nil を渡す。
...
- **Mutual の扱い**: `Mode = EndpointModeMutual` を関数内部で受け付けるが、本流 caller は当面ゼロ。dualstack 経由および将来 WS Mutual 要件が出たときの拡張点として残す。
...
type WebSocketConfig struct {
    ...
    TLS *tls.Config
    // Mode selects Client / Server / Mutual semantics:
    //   - Client:  no accept handler is registered; mux argument must be nil
    //   - Server:  accept handler registered on mux at cfg.Path; no outbound dial
    //   - Mutual:  both (accept + dial), shared rawSess
    Mode objproto.EndpointMode
}

// WebSocketEndpoint constructs a WebSocket-backed objproto Endpoint.
// mux is required when cfg.Mode is Server or Mutual; pass nil for Client.
func WebSocketEndpoint(mux *http.ServeMux, cfg WebSocketConfig) (objproto.Endpoint, error)

// WebSocketEndpointEx is the lower-level variant for callers that want to
...
### 誤用ガード (run-time check)

`WebSocketEndpoint` / `WebSocketEndpointEx` 内で以下の組み合わせを早期 error で弾く:

- `cfg.Mode == EndpointModeClient` かつ `mux != nil` → "mux must be nil for Client mode"
- `cfg.Mode in {Server, Mutual}` かつ `mux == nil` → "mux is required for Server/Mutual mode"
- 上記以外 (未知の Mode) → "unknown EndpointMode: ..."
...
type UDPWebsocketDualStackConfig struct {
    ...
    WS      WebSocketConfig      // Mode / Path / TLS / Logger をそのまま流用 (cfg.Logger との重複は許容)
}
...
`UDPWebsocketDualStackConfig` 自身に `Mode` フィールドを持たせず、`WS.Mode` で表現する（重複防止）。dualstack の内部実装は `WebSocketEndpointEx(rawSess, cfg.Mux, cfg.WS)` 1 行で済み、mode 別の switch は transport 内部に閉じる。caller 不在のまま新 API に追従するだけ（コード形態をドキュメントとして保存）。
...
| `server/server.go:238` | 旧 `transport.WebSocketEndpoint(logger, addr, tlsConf, sessMode)` 呼びを廃止し、`server.Run` 内部で `mux := http.NewServeMux()` → `WebSocketEndpoint(mux, WebSocketConfig{..., Mode: EndpointModeServer})` → `http.Server` 構築 + `ListenAndServe` を所有 |
| `cli/client.go:38` | `WebSocketEndpoint(nil, WebSocketConfig{Logger: ..., Path: cli.WebSocketPath, Mode: EndpointModeClient})` に書き換え |
...
| ファイル | build tag | 責務 |
|---|---|---|
| `transport/websocket_wasm.go` (新規) | `//go:build js` | `syscall/js` 経由でブラウザ WebSocket API をラップ。`WebSocketEndpoint(Ex)` を Client モード限定で実装 (Server / Mutual は wasm 環境では成立しないので mux 必須の case を error 返却) |
...
| 8 | wasm | `transport.WebSocketEndpoint(nil, WebSocketConfig{Path: cli.WebSocketPath, Mode: EndpointModeClient})` |
