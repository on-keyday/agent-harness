...
// Decoder converts one line of agent stdout into zero or more events. A line
// it cannot interpret yields exactly one KindRaw event holding the line
// verbatim. Decode never returns an error: a malformed line must not fail the
// task.
