...
func TestFileTransferE2E(t *testing.T) {
	...
	if err := c.FileLs(ctx, taskID, "", protocol.DataPlaneRoute_Splice, &lsBuf); err != nil {
		t.Fatalf("ls: %v", err)
	}
	...
	if err := c.FilePull(ctx, taskID, "uploaded.bin", dstPath, cli.FileTransferRange{}, false, protocol.DataPlaneRoute_Splice); err != nil {
		t.Fatalf("pull: %v", err)
	}
	...
	slice, total, err := c.FilePullBytesRange(ctx, taskID, "uploaded.bin",
		cli.FileTransferRange{Offset: 6, Length: 3}, protocol.DataPlaneRoute_Splice, nil)
	if err != nil {
	...
	empty, total, err := c.FilePullBytesRange(ctx, taskID, "uploaded.bin",
		cli.FileTransferRange{Offset: 9999}, protocol.DataPlaneRoute_Splice, nil)
	if err != nil {
	...
	if err := c.FilePull(ctx, taskID, "nope.bin", dstPath, cli.FileTransferRange{}, false, protocol.DataPlaneRoute_Splice); err == nil {
	...
	if err := c.FileMkdir(ctx, taskID, "newdir/made", false, protocol.DataPlaneRoute_Splice); err != nil {
		t.Fatalf("strict mkdir: %v", err)
	}
	if err := c.FileMkdir(ctx, taskID, "newdir/made", false, protocol.DataPlaneRoute_Splice); err == nil || !cli.IsAlreadyExists(err) {
		t.Fatalf("strict mkdir repeat: got %v, want already_exists", err)
	}
	// -p mkdir: deep chain + idempotent.
	if err := c.FileMkdir(ctx, taskID, "p1/p2/p3", true, protocol.DataPlaneRoute_Splice); err != nil {
	...
	if err := c.FileMkdir(ctx, taskID, "q1/q2/q3", false, protocol.DataPlaneRoute_Splice); err == nil || !cli.IsNotFound(err) {
...
func TestFileDirTransferE2E(t *testing.T) {
	...
	if err := c.FilePullDir(ctx, taskID, "incoming", localDst, false, protocol.DataPlaneRoute_Splice); err != nil {
