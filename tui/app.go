...
func (a *App) runAction(act Action) (tea.Model, tea.Cmd) {
	...
	case HelpAction:
		...
		a.cmdresult.Append("  --detach       leave what the command starts running after it ends (nothing reaps it but `exec kill`)")
		a.cmdresult.Append("  --sshd-parent  give the line a parent process named sshd, for a client that checks its ancestry (Windows; needs --shell)")
